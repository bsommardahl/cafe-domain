...
function calcDiscountTotal(item, discounts) {
  return Belt_List.reduce(discounts, 0, (function (acc, dis) {
                var rate = 1 + dis[/* percent */2] / 100;
                var discounted = item[/* suggestedPrice */2] / rate;
                var disc = item[/* suggestedPrice */2] - discounted;
                return acc + disc / 100;
              }));
}
...
function calcDiscountSubTotal(item, discounts) {
  return Belt_List.reduce(discounts, 0, (function (acc, dis) {
                var rate = dis[/* percent */2] / 100;
                var disc = item[/* suggestedPrice */2] / 100 * rate;
                return acc + disc;
              }));
}

function subTotalFirstCalculator(taxPercent, discounts, item) {
  var discountCalc = calcDiscountSubTotal(item, discounts);
...
exports.calcDiscountSubTotal = calcDiscountSubTotal;
